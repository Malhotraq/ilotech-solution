...
function getPool() {
  ...
  if (!globalThis.__ilotechPool) {
    ...
    // Tanpa handler ini, error koneksi idle (mis. DB restart) melempar
    // 'error' tak tertangani dan me-crash proses Node.
    globalThis.__ilotechPool.on('error', (err) => {
      console.error('PG pool idle error:', err?.message || err);
    });
  }
